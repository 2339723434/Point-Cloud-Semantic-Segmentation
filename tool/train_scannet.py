...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (coord, feat, target, offset) in enumerate(train_loader):  # [N, 3], [N, C], [N], [B]
        ...
        coord, target, feat, offset = \
            coord.cuda(non_blocking=True), target.cuda(non_blocking=True), feat.cuda(non_blocking=True), offset.cuda(non_blocking=True)

        
        #print(coord.shape,max(coord[:,0]),min(coord[:,0]),max(coord[:,1]),min(coord[:,1]),offset)
        #exit(0)
        output = model([coord, feat, offset])
        loss = criterion(output, target)
        #print(output.shape,output[0],target.shape)
        #print(loss)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        output = output[:, 1:].max(1)[1] + 1 if 'ScanNet_train' in args.dataset else output.max(1)[1]  # remove unclassified label
