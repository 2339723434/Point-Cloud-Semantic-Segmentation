...
def collate_fn(batch):
    ...
    for item in coord:
        count += item.shape[0]
        offset.append(count)
    #print(type(coord),type(coord[0]),coord[0].shape)
    #print(offset)
    
#    coord = torch.stack(list(coord), dim=0)
#    feat = torch.stack(list(feat), dim=0)
#    label = torch.stack(list(label), dim=0)
    return torch.cat(coord), torch.cat(feat), torch.cat(label) if label[0] is not None else None, torch.IntTensor(
        offset)



def block_data_prepare(coord, feat, label, args, split, coord_transform, rgb_transform,
